...
class PayloadConfig {
	constructor(endpoint) {
		...
		this.formatter =
			...
			endpoint.global[this.type + "Format"] ||
			(x => x)
		this.raw = undefined
	...
	getValidator(method) {
		...
		if (defaultMethods.includes(method))
			return defaultValidator(api, method, symbol)
		else return nonDefaultValidator(api, method, symbol)
	}
...
class QueryConfig extends PayloadConfig {
	...
	getValidator() {
		let validator = super.getValidator()
		if (validator === null) validator = super.getValidator(s.END)
		return validator
	}
}
